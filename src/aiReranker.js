const DEFAULT_POLICY = [
  ...
  'Respect deterministic eligibility, availability adjustments, and roster caps. The deterministic engine remains authoritative for who is eligible.',
  'Treat current availability risk as meaningful; do not restore a heavily discounted player to the top without a compelling roster reason.',
...
function candidatePayload(player, deterministicRank) {
  return {
    ...
    availabilityStatus: player.availabilityStatus ?? 'normal',
    availabilityMultiplier: player.availabilityMultiplier ?? 1,
    availabilityReason: player.availabilityReason ?? null,
    availabilityUpdatedAt: player.availabilityUpdatedAt ?? null,
    qb2QualityScore: player.components?.qb2QualityScore ?? null,
    qb2QualityMultiplier: player.components?.qb2QualityMultiplier ?? 1,
...
export function deriveTurnPairFromAiRanking(scoredPlayers, pairs = []) {
  const first = scoredPlayers?.[0];
  if (!first || !pairs.length) return null;

  // The deterministic pair engine has already simulated roster state after each
  // possible first pick. Once AI chooses the first player, use the best of those
  // sequential simulations for pick two instead of constraining AI to the old
  // deterministic pair list before it has had a chance to rerank the board.
